...
%Set MOVIE to true to turn on the animation
MOVIE =false;
...
dist = zeros(1,mySim.idx_end);
coll = zeros(1,mySim.idx_end);
...
while mySim.sim_flag == true
    ...
    drone1.u.Height = 10;
    drone1.u.AirSpeed = 5;
    drone1.u.RollAngle = 0;
    drone2.u.Height = 10;
    drone2.u.AirSpeed = 5;
    drone2.u.RollAngle = 0;
    ...
    if (Flight_Plan_1.State == 0)
        ...
        if (h1>17) %If __________
            ...
            Flight_Plan_1 = update_waypoints(Flight_Plan_1,waypoints_1,drone1);
        end
    ...
    elseif (Flight_Plan_1.State == 1)
        ...
        if Flight_Plan_1.waypoint_target>Flight_Plan_1.n_waypoints
            ...
            Flight_Plan_1.circle_center = [pn1,pe1];
        end
    ...
    elseif (Flight_Plan_1.State ==2)
        ...
        %Output
        drone1.u.Height = change_altitude(-30*mySim.dt,10,drone1);
        drone1.u.AirSpeed = 8;
        drone1.u.Xi_c = calculateCommand(flag,r,q,c,rho,lambda,drone1);
    ...
    elseif (Flight_Plan_1.State == Flight_Plan_1.LANDING)
        ...
        drone1.u.Height = 4;
        drone1.u.AirSpeed = 4;
        drone1.u.Xi_c = deg2rad(-30);
            
        %Trigger
        if (h1<5) %If __________
            Flight_Plan_1.State = Flight_Plan_1.FLARE;
            mySim.start_loop_time1 = mySim.t; %Restart timer
        end
        
    %State FLARE - Slowdown on approach %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    elseif (Flight_Plan_1.State == Flight_Plan_1.FLARE)
        %calculations
        mySim.loop_time1 = mySim.t - mySim.start_loop_time1; %Check timer    
        
        %Output
        drone1.u.Height = change_altitude(-10*mySim.dt,0,drone1);
    ...
    if (Flight_Plan_2.State == 0)
        ...
        if (h2>17) %If __________
            ...
            Flight_Plan_2 = update_waypoints(Flight_Plan_2,waypoints_2,drone2);
        end
    ...
    elseif (Flight_Plan_2.State == 1)
        ...
        if Flight_Plan_2.waypoint_target>Flight_Plan_2.n_waypoints
            ...
            Flight_Plan_2.circle_center = [pn2,pe2];
        end
    ...
    elseif (Flight_Plan_2.State ==2)
        ...
        %Output
        drone2.u.Height = change_altitude(-30*mySim.dt,10,drone2);
        drone2.u.AirSpeed = 8;
        drone2.u.Xi_c = calculateCommand(flag,r,q,c,rho,lambda,drone2);
    ...
    elseif (Flight_Plan_2.State == Flight_Plan_2.LANDING)
        ...
        drone2.u.Height = 4;
        drone2.u.AirSpeed = 4;
        drone2.u.Xi_c = deg2rad(-30);
        
        
        %Trigger
        if (h2<5) %If __________
            Flight_Plan_2.State = Flight_Plan_2.FLARE;
            mySim.start_loop_time2 = mySim.t; %Restart timer
        end
    
    %State FLARE - Slowdown on approach %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    elseif (Flight_Plan_2.State == Flight_Plan_2.FLARE)
        %calculations
        mySim.loop_time2 = mySim.t - mySim.start_loop_time2; %Check timer
                
        %Output
        drone2.u.Height = change_altitude(-10*mySim.dt,0,drone2);
        drone2.u.AirSpeed = change_speed(-10*mySim.dt,1,drone2);
    ...
    dist(i) = norm([pn1;pe1]-[pn2;pe2]); %2D distance
    coll(i) = norm([pn1;pe1;h1]-[pn2;pe2;h2]); %3D distance
    
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Only change in this section
    epx1 = ((pn2 - pn1) * cos(Xi1)) + ((pe2 - pe1) * sin(Xi1));
    epx2 = ((pe2 - pn1) * cos(Xi2)) + ((pe2 - pe1) * sin(Xi2));
    epy1 = (-(pn2 - pn1) * sin(Xi1)) + ((pe2 - pe1) * cos(Xi1));
    epy2 = (-(pn2 - pn1) * sin(Xi2)) + ((pe2 - pe1) * cos(Xi2));
    
    if (coll(i) < 35)        
        if (epx1 > 0) && (epx1 < 50)
           if (abs(epy1) < 1)
               drone1.u.Height = drone1.u.Height-10;
               if (drone1.u.Height < 20)
                   drone1.u.Height = 15;
               end
           else
               drone2.u.Height = drone2.u.Height - 10;
               if (drone2.u.Height < 20)
                   drone2.u.Height = 15;
               end
           end
        end
...
figure;
plot(drone1.data.x(2,1:i),drone1.data.x(1,1:i));
hold on;
axis equal;
title({'No Crashing','But they still want to be close'})
...
plot(drone2.data.x(2,1:i),drone2.data.x(1,1:i),'r');

if MOVIE
    make3DMovie2("Two-Example",drone1,drone2,i,40);
end
%%

figure;
plot(drone1.data.x(3,1:i),'b');
hold on;
plot(drone1.data.u(1,1:i),'b--');

plot(drone2.data.x(3,1:i),'r');
plot(drone2.data.u(1,1:i),'r--');
...
xlabel('Time (s)');
ylabel('Distance (m)');
legend('Distance between Drones','Threshold');
...
for i = 1:movie_length
    ...
    clf('reset');
    linestart = max([1 i-30]);
    plotTransforms(translations1(i*scale,:),rotations1(i*scale,:),'MeshFilePath','fixedwing.stl','InertialZDirection',"down",'FrameSize',10,'MeshColor',"blue")
    hold on;
    ...
    plotTransforms(translations2(i*scale,:),rotations2(i*scale,:),'MeshFilePath','fixedwing.stl','InertialZDirection',"down",'FrameSize',10,'MeshColor',"red");
...
obstacle_map(1,:) = 1;
obstacle_map(end,:) = 1;
obstacle_map([1:300, 510:800],1) = 1;
obstacle_map([1:300, 510:800],end) = 1;

obstacle_map(380:420,200:800) = 1;
obstacle_map(280:520,[180:210 300:330 420:450 540:570 660:690 780:810]) = 1;

obstacle_map(1:120, [180:210 300:330 420:450 540:570 660:690 780:810]) = 1;

obstacle_map(end-120:end, [180:210 300:330 420:450 540:570 660:690 780:810]) = 1;

%Create Image Version (flip and invert)
map = obstacle_map;
...
function waypoints = generateRRT(map,NW,SE,starting_point,ending_point)
    ...
    nodes = 1000; %Max number of test nodes
